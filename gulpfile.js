...
function copy_net_types() {
  return gulp.src(["scripts/types/**/*"]).pipe(gulp.dest("node_modules/@types"));
}

function copy_pack_icon() {
  return gulp.src(["pack_icon.png"]).pipe(gulp.dest("build/behavior_packs/codexworld"));
}

const copy_content = gulp.parallel(copy_behavior_packs, copy_resource_packs, copy_net_types, copy_pack_icon);
